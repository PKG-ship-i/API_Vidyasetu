...
namespace VidyasetuAPI.Controllers
{
	...
	public class GenerateResponseController : ControllerBase
	{
		...
		public GenerateResponseController(VidyasetuAI_DevContext db, HelperService helperService, IConfiguration config, IAccessValidatorService accessValidatorService)
		{
			_db = db;
		...
		public async Task<IActionResult> GenrateQuestionnaireFromVideoURL([FromBody] GenerateVideoRequestModel dto)
		{
			try
			{
				...
				// validate access
				int logCount = await _db.DeviceLogDetails.CountAsync(log => log.DeviceId == device.Id);
				var validationResponse = await _accessValidatorService.ValidateTrialLimitAsync(device, logCount);
